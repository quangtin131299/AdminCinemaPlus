...
router.get("/danhsachve", function(req, res){
	...
	conn.query(querysoluong, function(err, result){
		...
		if(err){
			...
			let query = `SELECT vedat.ID, DATE_FORMAT(vedat.NgayDat, '%d/%m/%Y') as 'NgayDat', DATE_FORMAT(suatchieu.Gio, '%H:%i') as 'Gio'
								, ghe.TenGhe
								, phim.TenPhim
								, khachhang.HoTen
								, rapphim.TenRap
								, phong.TenPhong
								, vedat.Status
						 FROM vedat JOIN suatchieu ON vedat.ID_Suat = suatchieu.ID
						 			 JOIN ghe ON ghe.ID = vedat.ID_Ghe
									 JOIN phim ON phim.ID = vedat.ID_Phim
									 JOIN khachhang ON khachhang.ID = vedat.ID_KhachHang 
									 JOIN rapphim ON rapphim.ID = vedat.ID_Rap 
									 JOIN phong ON phong.ID = vedat.ID_Phong limit ${vitribatdaulay}, 5`
